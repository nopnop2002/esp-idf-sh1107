	...
	while(1) {
		...
		for (int imageIndex=1; imageIndex<=10; imageIndex++) {
			...
			for (int digit=3;digit>=0;digit--) {
				int startPage = digit * 4; // startPage=12->8->4->0

				for (int srcIndex=63;srcIndex>=0;srcIndex--) {
					// Get internal buffer
					sh1107_get_buffer(&dev, buffer);
					int srcPage = 0;
					for (int page=startPage;page<startPage+4;page++) {
						// Move the display to the right by 1 dot
						for (int seg=63;seg>0;seg--) {
							int bufferIndex = page*64+seg;
							buffer[bufferIndex] = buffer[bufferIndex-1];
							ESP_LOGD(TAG, "page=%d seg=%d bufferIndex=%d", page, seg, bufferIndex);
						}
						// Insert from src images to buffer
						buffer[page*64] = src[64*srcPage+srcIndex];
						srcPage++;
					} // end page
					// Set internal buffer
